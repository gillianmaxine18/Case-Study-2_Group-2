...
def plot_pivot_heatmap(pivot_df: pd.DataFrame, output_dir: str, margin_label: str = 'Total_Sum') -> None:
    ...
    # FIX: this previously dropped index/columns literally named 'Total',
    # but analytics.py builds the pivot with margins_name='Total_Sum', so
    # that drop silently matched nothing (errors='ignore' masked it) and
    # the margin row/column leaked into the heatmap.
